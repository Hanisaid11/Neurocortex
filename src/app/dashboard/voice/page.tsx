...
import { orVoiceAssistantMedicalQuery, type MedicalQuestionOutput } from "@/ai/flows/or-voice-assistant-medical-query"
...
export default function VoiceAssistantPage() {
  ...
  const handleToggleListening = () => {
    if (!isListening) {
      ...
      setTimeout(() => {
        handleSendMessage("What are the primary surgical landmarks for an endoscopic endonasal approach to the pituitary?")
      }, 3000)
    } else {
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground italic">"Hey Neuro, [question]..."</span>
          </div>
        </div>
              ...
              <div className="space-y-6">
                {chatHistory.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-20 text-center opacity-40">
                    ...
                    <p className="text-lg font-headline">Waiting for voice command...</p>
                    <p className="text-sm">Speak clearly into the microphone. AI will provide audible responses.</p>
                  </div>
                )}
                ...
                {isProcessing && (
                  <div className="flex justify-start">
                    <div className="bg-secondary/50 p-4 rounded-2xl rounded-tl-none border border-border flex items-center gap-3">
                      ...
                      <span className="text-xs text-muted-foreground">Consulting Knowledge Library...</span>
                    </div>
                  </div>
                )}
              </div>
               ...
               <div className="flex flex-col items-center gap-6">
                  <div className="relative">
                    ...
                    <Button 
                      ...
                      className={`w-20 h-20 rounded-full shadow-2xl transition-all duration-500 scale-100 active:scale-90 ${isListening ? 'pulse-animation' : ''}`}
                    >
                  ...
                  <div className="text-center">
                    <p className={`text-sm font-headline font-bold transition-all ${isListening ? 'text-accent' : 'text-muted-foreground'}`}>
                      {isListening ? 'LISTENING FOR MEDICAL QUERY...' : 'TAP TO BEGIN DICTATION'}
                    </p>
                  </div>
               </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-card/30 border-border/50">
            ...
            <CardContent className="space-y-2">
              <Button variant="secondary" className="w-full justify-start gap-2 h-9 text-xs" onClick={() => handleSendMessage("Start operative note dictation.")}>
                <Zap className="w-3 h-3" /> Dictate Operative Note
              </Button>
              <Button variant="secondary" className="w-full justify-start gap-2 h-9 text-xs" onClick={() => handleSendMessage("Log intraoperative event: successful clipping.")}>
                <Zap className="w-3 h-3" /> Log Surgical Event
              </Button>
              <Button variant="secondary" className="w-full justify-start gap-2 h-9 text-xs" onClick={() => handleSendMessage("Request peer review of current view.")}>
